	...
	net.createServer(function(socket) {
        ...
        socket.getMinerSoftware = function() {
            ...
            if (message.params.length > 0) {
                if (message.params.length == 1) {
                    ...
                    if (poolListener.getSafeNumber(message.params[1].toString()) !== 0) {
        ...
        socket.setNotifyObject = function(obj, now){
            if (obj) {
                ...
                let diffMs = (now - socket.lastNotifyTime);
                if (changed || clean_job || diffMs > config.notifyTimeoutMs) {
        ...
        socket.processBuffer = function() {
            ...
            while (di > -1) {
                ...
                try {
                    ...
                    console.log("miner", minerId, "invalid json message", "from ip", socket.peerEndpoint);
                    return;
                }
                ...
                if (obj.method == 'mining.subscribe') {
                    ...
                    if (!socket.subscribed) {
                        ...
                        console.log("miner", minerId, "using software (", socket.getMinerSoftware(), ")", "from ip", socket.peerEndpoint);
                    }
