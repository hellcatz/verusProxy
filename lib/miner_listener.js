	...
	net.createServer(function(socket) {
        ...
        socket.getPool = function() {
            ...
            if (socket.miner && poolConnections.has(socket.miner)) {
                console.log("WARN, using pool connection by miner address", socket.minerId);
                return poolConnections.get(socket.miner);
            }
            // fallback to first pool
            if (poolConnections.size > 0) {
                return poolConnections.get(poolConnections.keys().next().value);
            }
            // no pool connections at all
            return undefined;
        };
        socket.getExtraNonce = function() {
            let pool = socket.getPool();
            let extraNonce = pool.getExtraNonce();
            if (extraNonce.length < 11) { // max 5 bytes pool can use, we use 3 bytes
        ...
        socket.processBuffer = function() {
            ...
            if (pool === undefined) {
                ...
                socket.processBufferTimeout = setTimeout(() => { socket.processBuffer(); }, 250);
                if (waitingForFirstPool !== true) {
                    console.log("miner", minerId, "first pool connection required");
                    pool = poolListener.newPoolConnection(config);
                    socket.pool = pool;
                    waitingForFirstPool = true;
                } else {
                    console.log("miner", minerId, "waiting for first pool connection");
                }
                return;
            }
            ...
            if (pool.getIsConnected() === false) {
                ...
                if (poolConnections.has(tmpaddr)) {
                    ...
                    if (tmppool.getIsConnected() !== false) {
                        ...
                        socket.pool = undefined;
                    }
                }
            }
            // wait for pool subscribe to complete
            if (pool.getSubscribeObjectResponse() === undefined) {
            ...
            if (socket.authorized && socket.subscribed && (!socket.shareTargetObject || !socket.notifyObject)) {
            ...
            while (di > -1) {
                ...
                if (obj.method == 'mining.subscribe') {
                    ...
                    if (!socket.miner) {
                        ...
                        if (!authObj) {
                            console.log("miner", minerId, "authorizing pool connection for miner address", socket.miner, socket.peerEndpoint);
                        ...
                        // we are no longer waiting for the first pool connection to come alive
                        waitingForFirstPool = false;
                    ...
                    if (!socket.extraNonceSubscribed) {
                        ...
                        if (!pool.isExtraNonceSubscribed()) {
                            pool.sendExtraNonceSubscribe(obj);
                        }
                    ...
                    socket.lastIds.pop(); // we are handling, pop the id
                    console.log("unknown method from miner", obj);
